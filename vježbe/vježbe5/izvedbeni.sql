...
CREATE TABLE STUDIJSKI_PROGRAM (
    ST_PROG_ID INTEGER PRIMARY KEY,
...
CREATE TABLE IZVEDBENI_PLAN (
    ...
    ST_PROG_ID INTEGER NOT NULL REFERENCES STUDIJSKI_PROGRAM (ST_PROG_ID),
...
SELECT DISTINCT d.djelatnik_id, d.ime, d.prezime
FROM djelatnik d LEFT OUTER JOIN djelatnik_izvedbeni di
USING (djelatnik_id)
...
FROM kolegij k LEFT OUTER JOIN studijski_program sp USING (kolegij_id)
LEFT OUTER JOIN izvedbeni_plan ip USING (st_prog_id)
WHERE ip.st_prog_id IS NULL;

-- Zadatak 8
--  Ispišite duljinu imena svakog djelatnika iz baze za nastavu.
SELECT length(ime), ime
FROM djelatnik;
...
SELECT P :: TEXT, V :: TEXT, S::TEXT, TO_CHAR(S, '0.0')
FROM KOLEGIJ;
...
-- Zadatak 14
-- Za unesene smjerove napišite upit koji će vratiti naziv smjera, te posebno broj kolegija koji su obvezni i broj onih koji nisu obvezni 
-- (dva retka za svaki studij, jedan obvezni drugi izborni, i u rezultatima umjesto vrijednosti u stupcu obvezni treba pisati obvezni ili izborni) 
-- te maksimalni i minimalni broj ECTS bodova (po smjeru i po tome je li izborni ili ne). Rezultate poredajte po nazivu studija.
SELECT * FROM studijski_program;

SELECT vs.naziv, 
CASE WHEN sp.obvezni = 1 THEN 'obvezni' ELSE 'izborni' END AS tip,
COUNT(DISTINCT k.kolegij_id) AS broj, min(k.ects) AS "min(ECTS)", max(k.ects) AS "max(ECTS)"
...
-- Zadatak 15
-- Napišite upit koji će vratiti naziv studija, godinu studija i semestar za sve one semestre na odredenoj godini studija kod kojih obvezni kolegiji 
-- imaju ukupan broj ECTS bodova (u semestru) veći ili jednak 20. Rezultate poredajte po nazivu studija, a zatim po godini.
SELECT vs.naziv, sp.godina, ip.semestar
FROM vrsta_studija vs INNER JOIN studijski_program sp USING (vrsta_studija_id)
